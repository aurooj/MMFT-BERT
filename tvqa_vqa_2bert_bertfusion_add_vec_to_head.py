...
class ABC(nn.Module):
    def __init__(self, opt):
        ...
        if self.add_src_vec:
            self.fusion_dim = self.embedding_size + opt.src_vec_dim*opt.n_heads
    ...
    def forward(self, **batch):
        ...
        if self.sub_flag:
            ...
            if self.add_src_vec:
                ...
                e_QA = []
                for e_qa in [e_a0, e_a1, e_a2, e_a3, e_a4]:
                    e_qa = list(torch.chunk(e_qa, self.n_heads, dim=-1))
                    e_newqa = [torch.cat([e_a, src_vec_q], -1) for e_a in e_qa]
                    e_qa = torch.stack(e_newqa, -1).view(bsz, -1)
                    e_QA.append(e_qa)
                ...
                e_VQA = []
                for e_vqa in [e_vqa0, e_vqa1, e_vqa2, e_vqa3, e_vqa4]:
                    e_vqa = list(torch.chunk(e_vqa, self.n_heads, dim=-1))
                    e_newvqa = [torch.cat([e_a, src_vec_v], -1) for e_a in e_vqa]
                    e_vqa = torch.stack(e_newvqa, -1).view(bsz, -1)
                    e_VQA.append(e_vqa)

                e_vqa0, e_vqa1, e_vqa2, e_vqa3, e_vqa4 = e_VQA[0], e_VQA[1], e_VQA[2], e_VQA[3], e_VQA[4]

                e_SQA = []
                for e_sqa in  [e_sqa0, e_sqa1, e_sqa2, e_sqa3, e_sqa4]:
                    e_sqa = list(torch.chunk(e_sqa, self.n_heads, dim=-1))
                    e_newsqa = [torch.cat([e_a, src_vec_s], -1) for e_a in e_sqa]
                    e_sqa = torch.stack(e_newsqa, -1).view(bsz, -1)
                    e_SQA.append(e_sqa)
