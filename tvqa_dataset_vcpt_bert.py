...
class TVQADataset(Dataset):
    def __init__(self, opt, mode="train"):
        ...
        self.max_seq_len = opt.max_vcpt_l
    ...
    def __getitem__(self, index):
        ...
        if self.with_ts:
            ...
            q_vcpt = self.numericalize_vcpt(cur_vis_sen, ques)

            items.append(q1_vcpt)
            items.append(q2_vcpt)
            items.append(q_vcpt)

            vcpt_a0 = self.numericalize_vcpt(cur_vis_sen ,self.cur_data_dict[index]['a0'])
            vcpt_a1 = self.numericalize_vcpt(cur_vis_sen, self.cur_data_dict[index]['a1'])
            vcpt_a2 = self.numericalize_vcpt(cur_vis_sen, self.cur_data_dict[index]['a2'])
            vcpt_a3 = self.numericalize_vcpt(cur_vis_sen, self.cur_data_dict[index]['a3'])
            vcpt_a4 = self.numericalize_vcpt(cur_vis_sen, self.cur_data_dict[index]['a4'])

            items.extend([vcpt_a0, vcpt_a1, vcpt_a2, vcpt_a3, vcpt_a4])
    ...
    def numericalize_vcpt(self, vcpt_sentence, q):
        ...
        # sentence_indices = [self.word2idx[w] if w in self.word2idx else self.word2idx["<unk>"]
        #                     for w in words]
        sentence =  " ".join(words) + ". " + q
        sentence = self.trunc_if_gt_max_seq_len(sentence, self.max_seq_len)
        sentence_indices = self.tokenizer.encode(sentence, add_special_tokens=True)
        return sentence_indices

    def trunc_if_gt_max_seq_len(self, vqa, max_seq_len=300):
        full_len = len(vqa.split(" "))
        if full_len > max_seq_len:
            vqa_split = vqa.split(".")
            v = vqa_split[0]
            len_adjust_idx = full_len - max_seq_len + 2 + len(vqa_split[1].split(" "))
            # len_adjust_idx = len(v.split(" ")) - len_adjust_idx
            if len_adjust_idx < 0:
                len_adjust_idx = len_adjust_idx * -1
            v = v.split(" ")[:-len_adjust_idx]
            # vqa = v + vqa_split[1]
            vqa = " ".join(v) + " . "+ vqa_split[1]
            if len(vqa.split(" ")) >= max_seq_len: print(len(vqa.split(" ")))
        return vqa
...
def pad_collate(data):
    ...
    text_keys = ["q1_a0", "q2_a0", "q1_a1", "q2_a1", "q1_a2", "q2_a2", "q1_a3", "q2_a3", "q1_a4", "q2_a4", "q_a0",
                 "q_a1", "q_a2", "q_a3", "q_a4", "sub", "q1_vcpt", "q2_vcpt", "vcpt", "v0", "v1", "v2", "v3", "v4"]
...
def preprocess_inputs(batched_data, max_sub_l, max_vcpt_l, max_vid_l, device="cuda:0"):
    ...
    max_len_dict = {"sub": max_sub_l, "vcpt": max_vcpt_l, "vcpt_ts":max_vcpt_l, "vid": max_vid_l, "q1_vcpt":max_vcpt_l,
                    "q2_vcpt":max_vcpt_l, "v0": max_vcpt_l, "v1": max_vcpt_l, "v2": max_vcpt_l, "v3": max_vcpt_l, "v4": max_vcpt_l}
    text_keys = ["q1_a0", "q2_a0", "q1_a1", "q2_a1", "q1_a2", "q2_a2", "q1_a3", "q2_a3", "q1_a4", "q2_a4", "q_a0",
                 "q_a1", "q_a2", "q_a3", "q_a4", "sub",  "q1_vcpt", "q2_vcpt", "vcpt", "v0", "v1", "v2", "v3", "v4"]
    label_key = "answer_idx"
    bow_key = 'bow'
