import os
from fine_tune_bert.bert_based_model.context_query_attention import StructuredAttention
from fine_tune_bert.bert_based_model.encoder import StackedEncoder

from model.bidaf import BidafAttn
from model.model2 import MACUnit
from model.model_qa import MACUnit as MAC_QA, SelfAttentionUnit, TwoLayerSelfAttention, linear
...
class ABC(nn.Module):
    def __init__(self, opt):
        ...
        # config = config_class.from_pretrained('/data/Aisha/TVQA/Reason-VQA/fine-tune-bert/output/config.json')
        ...
        if self.vcpt_flag:
            ...
            self.setattn = BidafAttn(self.embedding_size, method="dot", get_h=True)
    ...
    def forward(self, q, q_l, qm1, qm_l1, qm2, qm_l2,  a0, a0_l, a1, a1_l, a2, a2_l, a3, a3_l, a4, a4_l,
        ...
        e_a0 = self.embedding(a0)[1][-2]
        e_a1 = self.embedding(a1)[1][-2]
        e_a2 = self.embedding(a2)[1][-2]
        e_a3 = self.embedding(a3)[1][-2]
        e_a4 = self.embedding(a4)[1][-2]
        ...
        raw_out_q = self.base_encoder(e_q, self.bert_word_encoding_fc, self.input_embedding, self.input_encoder)
        hq = raw_out_q[:,0]
        #todo: use q+a instead of separate answers
        raw_out_a0 = self.base_encoder(e_a0, self.bert_word_encoding_fc, self.input_embedding, self.input_encoder)
        raw_out_a1 = self.base_encoder(e_a1, self.bert_word_encoding_fc, self.input_embedding, self.input_encoder)
        raw_out_a2 = self.base_encoder(e_a2, self.bert_word_encoding_fc, self.input_embedding, self.input_encoder)
        raw_out_a3 = self.base_encoder(e_a3, self.bert_word_encoding_fc, self.input_embedding, self.input_encoder)
        raw_out_a4 = self.base_encoder(e_a4, self.bert_word_encoding_fc, self.input_embedding, self.input_encoder)
        ...
        if self.vcpt_flag:
            ...
            e_vcpt = self.embedding(vcpt)[1][-2]
            e_vcpt = self.base_encoder(e_vcpt, self.bert_word_encoding_fc, self.input_embedding, self.input_encoder)
        ...
        print(out.shape)
        return out, prob_matrix, q_attn, attn, similarities, None, s
    ...
    def stream_processor(self, lstm_mature, classifier, ctx_embed, ctx_l,
        ...
        return out, prob_matrix, q_attn, attn, None
