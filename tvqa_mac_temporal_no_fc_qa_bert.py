...
class ABC(nn.Module):
    def __init__(self, opt):
        ...
        # self.fc = nn.Sequential(linear(self.embedding_size, 256),
        #                             # nn.ReLU()
        #                             )
        self.classifier = linear(self.embedding_size, 5)
    ...
    def forward(self, q, q_l, qm1, qm_l1, qm2, qm_l2,  a0, a0_l, a1, a1_l, a2, a2_l, a3, a3_l, a4, a4_l,
        ...
        QA = self.embedding(q)[1][-1][:,0]
        # e_q = self.embedding(q)[1][-2]
        # e_a0 = self.fc(self.embedding(a0)[1][-1][:,0]) # 0 for CLS token
        # e_a1 = self.fc(self.embedding(a1)[1][-1][:,0])
        # e_a2 = self.fc(self.embedding(a2)[1][-1][:,0])
        # e_a3 = self.fc(self.embedding(a3)[1][-1][:,0])
        # e_a4 = self.fc(self.embedding(a4)[1][-1][:,0])



        # QA = torch.cat([e_a0, e_a1, e_a2, e_a3, e_a4], dim=-1)
        scores = self.classifier(QA)
