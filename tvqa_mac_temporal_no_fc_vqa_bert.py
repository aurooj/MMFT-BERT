...
class ABC(nn.Module):
    ...
    def forward(self, q, q_l, qm1, qm_l1, qm2, qm_l2,  vqa0, vqa0_l, vqa1, vqa1_l, vqa2, vqa2_l,
                vqa3, vqa3_l, vqa4, vqa4_l,
       ...
       # visual_attn_embed = visual_attn_embed.sum(1).squeeze() # sum over N to get B, 768
        ...
        e_vqa0 = self.vqa_embedding(vqa0)[1][-4][:,0] # 0 for CLS token
        e_vqa1 = self.vqa_embedding(vqa1)[1][-4][:,0]
        e_vqa2 = self.vqa_embedding(vqa2)[1][-4][:,0]
        e_vqa3 = self.vqa_embedding(vqa3)[1][-4][:,0]
        e_vqa4 = self.vqa_embedding(vqa4)[1][-4][:,0]
